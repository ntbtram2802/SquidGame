...
public class KeyHandler implements KeyListener {
    ...
    public void keyPressed(KeyEvent e) {
        ...
        if (GamePanel.gameState == GamePanel.playState) { // Khi gameState == playstate thi se dung bo key cua playste
            count = 0;
            if (GamePanel.pauseState == true) {
                optionmenu(code);
            } // if you pause in playing game, use key of pausegame
            else {
                playstate(code);
            }
        } else if (GamePanel.gameState == GamePanel.winState) { // playstate == win -> only use key of win
            winoption(code);
        } else if (GamePanel.gameState == GamePanel.loseState || gp.gameState == gp.endState) {
            loseoption(code);
